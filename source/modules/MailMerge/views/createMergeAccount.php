...
class MailMerge_createMergeAccount_View extends Vtiger_Index_View {
...
//for setting vtiger_accountid=0 for the contacts which are deleted
        ...
        $result = $db->pquery($ct_query, array());
        ...
        if (count($deleted_id) > 0) {
            ...
            $result = $db->pquery($update_query, array($deleted_id));
        }
...
//<<<<<<<<<<<<<<<<End>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        if (count($querycolumns) > 0) {
            ...
            $query = "select DISTINCT  $selectcolumns from vtiger_account 
        ...
        unlink($delefile1);
          ...
          //  echo $delefile;
            ...
            foreach ($mass_merge as $idx => $entityid) {
                 ...
                 $rs = $db->pquery("select * from vtiger_account where accountid=?", array($entityid));
       ...
       // $i = 0;
        if ($row = $db->fetch_row($rs)) {
            ...
            $a_id=$row['accountid'];
               ...
               // echo $wordtemplatedownloadpath.$entityid.$filename;
               // echo mime_content_type($wordtemplatedownloadpath.$entityid.$filename);
     ...
     // echo  filetype($path . $randomfilename . '.zip');
        //echo $attachmentsid;
            ...
            rename($wordtemplatedownloadpath.$entityid.$filename,$newfilepath );
                ...
                echo "&nbsp;&nbsp;<font size=+1><b><a href=$newfilepath class='btn btn-info' style='width:400px;'>" . 'Download Merged File (' .$aname.')'. "</a></b></font><br>";
